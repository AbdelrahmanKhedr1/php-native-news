...
if (!function_exists('route_get')) {
    function route_get($segment, $view = null)
    {
        ...
        $routes['GET'][] = [
            'view' => $view,
            'segment' => '/learn.php/' . public_() . '/' . ltrim($segment, '/'),
        ];
    }
}
...
if (!function_exists('url')) {
    function url($segment)
    {
        ...
        return $url . '/learn.php/' . public_() . '/' . ltrim($segment, '/');
    }
}

/**
 * Generate Admin URL for a given segment.
 *
 * @param string $segment The URL segment.
 * @return string The generated URL.
 */
if (!function_exists('aurl')) {
    function aurl($segment): string
    {
        $url = isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 'https://' : 'http://';
        $url .= $_SERVER['HTTP_HOST'];
        return $url . '/learn.php/' . public_() . '/' . ADMIN . '/' . ltrim($segment, '/');
    }
}
